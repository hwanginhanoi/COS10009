...
class GameWindow < Gosu::Window
  ...
  def print_cells
    for i in 0...MAP_WIDTH / CELL_DIM
      for j in 0...MAP_HEIGHT / CELL_DIM
        no = @columns[j][i].north == nil ? "0" : "1"
        so = @columns[j][i].south == nil ? "0" : "1"
        ea = @columns[j][i].east == nil ? "0" : "1"
        we = @columns[j][i].west == nil ? "0" : "1"
        puts "Cell x: " + (i).to_s + ", y: " + (j).to_s + " north:" + no + " south:" + so + " east:" + ea + " west:" + we
      end
      puts "------------ End of Column ------------"
    end
  end
